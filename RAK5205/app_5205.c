 ...
 */
uint8_t user_dev_eui[8] = {0x00,0x01,0x84,0xEF,0xC7,0xE7,0x41,0xE4};
uint8_t user_app_eui[8] = {0x70,0xB3,0xD5,0x7E,0xD0,0x03,0xA8,0x07};
uint8_t user_app_key[16] = {0x54,0xBD,0x99,0xBA,0x6F,0x0E,0x02,0xE1,0xA7,0xB9,0x00,0x26,0x15,0x8D,0xDF,0x63};
...
void setup(void)
{
    ...
    ret_code = rui_lora_set_region(US915);
    if (ret_code != RUI_STATUS_OK)
...
void main(void)
{
    rui_uart_mode_config(RUI_UART1,RUI_UART_USER); //Forces UART3 to be configured in RUI_UART_USER mode
    rui_init();
    bsp_init();
